...
let rec pattern p =
  group
    ...
    | PWildcard ->
        underscore
    | PTuple li ->
        parens (semicolon_list pattern li)
    | PVar name ->
        !^name
    | PPrimitive prim ->
        primitive prim
...
and type_ t =
  group
    ...
    | TPrimitive p ->
        primitive p
    | TApply (li, name) ->
        parens (comma_list type_ li) ^-^ !^name
    | TArrow (li, final) ->
        rarrow_list type_ li ^-^ rarrow ^|^ type_ final )

and named_arg (name, arg) = parens @@ !^"~" ^^ !^name ^^ colon ^^ expr arg
...
and expr e =
  group
    ...
    | EIf (cond, e1, e2) ->
        group
        @@ group (!^"if" ^|^ expr cond ^|^ !^"then")
        ^|^ expr e1
        ^|^ group (!^"else" ^|^ expr e2)
    | ELet (defs, e) ->
        group (space_list def defs) ^-^ expr e
    | EApply (f, named_args, args) ->
        parens (expr f)
    ...
    | ELitList exprs ->
        brackets (semicolon_list expr exprs)
    | ELitInt i ->
        PPrint.OCaml.int i
    | EVar name ->
        !^name
    ...
    | ESequence (li, e) ->
        semicolon_list expr li ^|^ semi ^-^ expr e
    | EPrimitive prim ->
        primitive prim
    | EOpenModule (m, e) ->
        string m ^^ dot ^^ lparen ^^ nest_break 0 (expr e) ^|^ rparen
    | EModuleField path ->
        dot_list string path
    | EMixedSequence (li, e) ->
        space_list mixed li ^|^ expr e
    | ELitString s ->
        OCaml.string s
    | EMatch (e, branches) ->
